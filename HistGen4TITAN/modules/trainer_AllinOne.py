...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch, model_name):
        ...
        # CHANGE: Variable name for clarity (images -> slide_embeddings)
        for batch_idx, (images_id, slide_embeddings, reports_ids, reports_masks) in enumerate(self.train_dataloader):
            slide_embeddings, reports_ids, reports_masks = slide_embeddings.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device)
            
            # CHANGE: Pass slide_embeddings instead of images
            output = self.model(slide_embeddings, reports_ids, mode='train')
        ...
        with torch.no_grad():
            ...
            # CHANGE: Variable name and model input
            for batch_idx, (images_id, slide_embeddings, reports_ids, reports_masks) in enumerate(self.val_dataloader):
                slide_embeddings, reports_ids, reports_masks = slide_embeddings.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device)
        ...
        # Test set evaluation (if needed)
        '''self.logger.info('[{}/{}] Start to evaluate in the test set.'.format(epoch, self.epochs))
        self.model.eval()
        with torch.no_grad():
            test_gts, test_res = [], []
            for batch_idx, (images_id, slide_embeddings, reports_ids, reports_masks) in enumerate(self.test_dataloader):
                slide_embeddings, reports_ids, reports_masks = slide_embeddings.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device)
                output = self.model(slide_embeddings, mode='sample')
                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
