...
class IdentityEncoder(nn.Module):
    """Simple identity encoder for TITAN embeddings"""
    def __init__(self):
...
class SimpleTitanTransformer(nn.Module):
    """Simplified transformer without complex hierarchical encoding"""
    def __init__(self, encoder, decoder, src_embed, tgt_embed, d_model):
        super(SimpleTitanTransformer, self).__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.src_embed = src_embed
        self.tgt_embed = tgt_embed
    ...
    def encode(self, src, src_mask):
        # Simple identity encoding for TITAN embeddings
        return self.encoder(self.src_embed(src), src_mask)
    
    def decode(self, memory, src_mask, tgt, tgt_mask, past=None):
        if past is None:
            # FIX: Return only the tensor, not a tuple
            return self.decoder(self.tgt_embed(tgt), memory, src_mask, tgt_mask)
        else:
            # For incremental decoding (used in sampling mode)
            return self.decoder(self.tgt_embed(tgt), memory, src_mask, tgt_mask, past=past)


class BaseTitanHistGen(AttModel):
    def make_model(self, tgt_vocab):
        """Simplified model without HATEncoder and CMC module"""
        c = copy.deepcopy
        attn = MultiHeadedAttention(self.num_heads, self.d_model)
        ff = PositionwiseFeedForward(self.d_model, self.d_ff, self.dropout)
        position = PositionalEncoding(self.d_model, self.dropout)
        
        # Simplified transformer with identity encoder (no complex hierarchical encoding)
        model = SimpleTitanTransformer(
            IdentityEncoder(),  # Simple identity mapping for TITAN embeddings
            Decoder(DecoderLayer(self.d_model, c(attn), c(attn), c(ff), self.dropout), self.num_layers),
            lambda x: x,  # Source embedding (identity for TITAN)
    ...
    def __init__(self, args, tokenizer):
        super(BaseTitanHistGen, self).__init__(args, tokenizer)
        self.args = args
        self.num_layers = args.num_layers
        self.d_model = args.d_model
        self.d_ff = args.d_ff
        ...
        # No CMC module - removed completely
        # No region_size or encoder_layout - not needed for TITAN
        # CRITICAL FIX: Override inherited att_embed to expect projection_dim (512) instead of 2048
        self.att_embed = nn.Sequential(
            nn.Linear(args.projection_dim, args.d_model),  # 512 → 512 (not 2048 → 512) FOR training 1. FROM Training 2 it is 768 → 768
            nn.ReLU(),
            nn.Dropout(self.drop_prob_lm)
        )
        
        print(f"DEBUG: BaseTitanHistGen att_embed expects input dim: {args.projection_dim}")
        print(f"DEBUG: BaseTitanHistGen att_embed outputs dim: {args.d_model}")

        # Also override fc_embed if it exists and is used
        if hasattr(self, 'fc_embed'):
            self.fc_embed = nn.Sequential(
                nn.Linear(args.projection_dim, args.d_model),  # 512 → 512
                nn.ReLU(), 
                nn.Dropout(self.drop_prob_lm)
            )
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        """Simplified feature preparation for TITAN embeddings"""
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks)
        
        # Simple encoding (identity mapping) since TITAN embeddings are already rich
        memory = self.model.encode(att_feats, att_masks)
        
        return fc_feats[..., :1], att_feats[..., :1], memory, att_masks

    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        """Simplified feature preparation without CMC interaction"""
        ...
        # REMOVED: All CMC module interactions
        # No memory querying, no prototype selection, no cross-modal augmentation
        # TITAN embeddings are used directly without enhancement
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        """Simplified forward pass without CMC module"""
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)
        
        # No memory_matrix parameter - CMC module removed
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        """Simplified core function without CMC module"""
        if len(state) == 0:
            ys = it.unsqueeze(1)
            past = [fc_feats_ph.new_zeros(self.num_layers * 2, fc_feats_ph.shape[0], 0, self.d_model),
                    fc_feats_ph.new_zeros(self.num_layers * 2, fc_feats_ph.shape[0], 0, self.d_model)]
        ...
        # No memory_matrix parameter - simplified decode call
        out, past = self.model.decode(memory, mask, ys, subsequent_mask(ys.size(1)).to(memory.device), past=past)
